...
const conf = convict({
  ...
  locale: {
    list: {
      ...
      env: 'LOCALE_LIST',
      format: Array,
      default: ['en-US', 'cy', 'tr', 'sv', 'es', 'sl', 'sk', 'gd', 'ru', 'rm',
                'pt-BR', 'pl', 'lt', 'lij', 'ko', 'ja', 'it', 'id', 'de', 'fy',
                'fr', 'et', 'nl', 'cs', 'zh-TW', 'zh-CN', 'ca', 'eu', 'it-CH']
    },
    base: {
...
function arrayEqual(arrA, arrB) {
  return (arrA === arrB) || (JSON.stringify(arrA) === JSON.stringify(arrB));
}

// unless specified, derive localeList from directory structure
if (arrayEqual(conf.get('locale.list'), conf.default('locale.list'))) {
  (function() {
    var dir = conf.get('locale.dir');
    if (dir && fs.existsSync(dir)) {
      conf.set('locale.list', fs.readdirSync(dir));
    }
  })();
}
