...
conf.set('personaUrl', conf.get('personaUrl').trim().replace(/[\s\/]*$/, ''));
conf.set('publicUrl', conf.get('publicUrl').trim().replace(/[\s\/]*$/, ''));

// Derive openid realm from publicUrl
//   https://foo.bar.example.com -> https://*.example.com
//   http://127.0.0.1:3000       -> http://127.0.0.1:3000
if (conf.get('openidRealm') === undefined) {
  // If only v8 supported lexical scoping...
  (function () {
    var publicUrl = url.parse(conf.get('publicUrl'));
    ...
    if ('port' in publicUrl) {
      conf.set('openidRealm', conf.get('publicUrl'));
    } else {
      conf.set('openidRealm', publicUrl.protocol + '//*.' + basename);
    }
  })();
}

// Derive default issuer from publicUrl
if (conf.get('issuerDomain') === undefined) {
  conf.set('issuerDomain', url.parse(conf.get('publicUrl')).hostname);
}
