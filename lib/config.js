...
const conf = convict({
  ...
  issuer: {
    ...
    env: 'ISSUER_HOSTNAME',
    format: String,
    default: 'localhost' // in production, should be url.parse(publicUrl).hostname
  },
  port: {
  ...
  sessionDuration: {
    doc: 'The duration of session cookies, in milliseconds. Must take into account clock skew and be long enough for the user to log into Google.',
...
if (conf.get('issuer') === 'localhost') {
  conf.set('issuer', 'gmail.com');
  console.warn('Issuer was "localhost", setting to "gmail.com" for testing.');
}
